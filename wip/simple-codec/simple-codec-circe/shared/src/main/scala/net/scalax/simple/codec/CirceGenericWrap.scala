...
trait EncoderWrap[F[_[_]], T] extends Encoder[T] {
  ...
  def simpleProduct3: SimpleProduct3.Appender[F]
  ...
  def toModel[U](u: U => T): EncoderWrap[F, U] = new EncoderWrap[F, U] {
    ...
    override def simpleProduct3: SimpleProduct3.Appender[F] = EncoderWrapSelf.simpleProduct3
...
object CirceGeneric {
  ...
  class encodeModelImpl[F[_[_]]](
    simpleProduct3: SimpleProduct3.Appender[F],
    ...
    override def apply(model: F[cats.Id]): Json = {
      ...
      val encodeFunc: EncodeJson[F[Named], F[Encoder], F[cats.Id]] =
        simpleProduct3.toHList1[EncodeJson, Named, Encoder, cats.Id](appender)(typeGen)
