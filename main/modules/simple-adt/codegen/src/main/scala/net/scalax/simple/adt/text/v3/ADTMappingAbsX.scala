...
class ADTMappingAbsX(val index: Int) {
  ...
  class TraitBody(val index: Int) {
    ...
    def typeParam3Impl(index: Int): String =
      if (index <= TraitBodySelf.index) s"APProduct2[M2[T$index, N$index], ${typeParam3Impl(index + 1)}]" else "Pro2"
    val typeParam3: String = typeParam3Impl(1)

    def typeParam4Impl(index: Int): String =
      if (index <= TraitBodySelf.index) s"APCoProduct1[M1[T$index, N$index], ${typeParam4Impl(index + 1)}]" else "Co1"
