...
object AppendTail6 {
  ...
  trait AppendToOption[TPoint, XU1 <: AdtCoProduct]
      extends AppenderAlias[({ type XM[U <: AdtCoProduct] = U => Option[TPoint] })#XM, AdtCoProduct, AdtCoProduct.UsePositive, XU1] {
    AppendToOptionSelf =>
  ...
  object AppendToOption extends LawImplicit {
    implicit def appendToZero[OneType, ZeroInstance <: AdtCoProduct]
  ...
  trait LawImplicit {
    implicit def appendToOption[T1, OT, ZT <: AdtCoProduct](implicit
  ...
  def toValue[OneType, ZeroInstance <: AdtCoProduct](
    m: AdtCoProduct.UsePositive[OneType, ZeroInstance]
  ): AdtCoProduct.UsePositive[OneType, ZeroInstance] = m

  trait Apply[T] {
    def take[A <: AdtCoProduct](a: A)(implicit k: AppendToOption[T, A]): Option[T] = k.current(a)
  }
