...
trait CoProductUtilN[CoLike1, ApCoProduct1[_, _ <: CoLike1] <: CoLike1, HLLike, ApHList[_, _ <: HLLike] <: HLLike, CoLike3, ApCoProduct3[
  _,
  _ <: CoLike3
] <: CoLike3] { CoProductUtil1Self =>
  ...
  trait FunctionAppend[M1[_, _], M2[_, _], M3[_, _]] { FunctionAppendSelf =>
    ...
    def paramSupport: Parameter10NatSupport3[
      ...
      override val typeGen: Type10Gen3[
        ...
        override def gen10[A, B, X1, X2, X3, X4, X5, X6, X7, X8, X9, X10, X11, X12, X13, X14, X15, X16, X17, X18, X19, X20]
          : (M1[A, B], M2[A, B]) => M3[A, B] = (a1: M1[A, B], a2: M2[A, B]) => FunctionAppendSelf.supportFunc.to[A, B](a1, a2)
      }
    }
  }
  ...
  def next[A1, A2, Co1 <: CoLike1, H1 <: HLLike, Co2 <: CoLike3](
    ...
    val i: ItemFunc[({ type IDF[T1, T2] = T1 })#IDF, ({ type Func1[T1, T2] = T1 => T2 })#Func1, ({ type IDF[T1, T2] = T2 })#IDF] =
      new ItemFunc[({ type IDF[T1, T2] = T1 })#IDF, ({ type Func1[T1, T2] = T1 => T2 })#Func1, ({ type IDF[T1, T2] = T2 })#IDF] {
        override def to[A1, A2](m1: A1, m2: A1 => A2): A2 = m2(m1)
      }

    val m = new CoProductUtil1Self.FunctionAppend[
      ({ type IDF[T1, T2] = T1 })#IDF,
      ({ type Func1[T1, T2] = T1 => T2 })#Func1,
      ({ type IDF[T1, T2] = T2 })#IDF
    ] {
      override def supportFunc
        : ItemFunc[({ type IDF[T1, T2] = T1 })#IDF, ({ type Func1[T1, T2] = T1 => T2 })#Func1, ({ type IDF[T1, T2] = T2 })#IDF] = i
    }
